...
<script>
	...
	$.ajax({
		url :"http://192.168.0.5:9080/recipedetail3.do",
		method:"GET",
		dataType:"json",
		success : function(data){
			if(data){
				 console.log(data)
		          $(".view_pic1").css("background-image","url("+data[2].img+")");
		          $("#name").text(data[2].name);
		          $("#recipeIntro").text(data[2].summary);
		          $(".view1_summary_info1").text(data[2].info.info1);
		          $(".view1_summary_info2").text(data[2].info.info2);
		          $(".view1_summary_info3").text(data[2].info.info3);
		          //console.log(data[0].ingre[0].ingre_name);
		          console.log(data[2].ingre.length);
		         for(var i=0;i<data[2].ingre.length;i++){
		        	 var dataString="<h3>"+data[2].ingre[i].ingre_name+"&nbsp;"+data[2].ingre[i].ingre_count+data[2].ingre[i].ingre_unit+"</h3>";
