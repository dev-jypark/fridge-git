...
console.log("jsp파일에서 id : "+id);
console.log('상세보기2들어옴');
var jsonId = {"id": id};
	$.ajax({
		//수정파트
		url :"http://192.168.35.89:9080/recipedetail2.do",
		method:"POST",
		dataType:"json",
		...
		success : function(data){
			if(data){
				 console.log(data)
		          $(".view_pic1").css("background-image","url("+data[1].img+")");
		          $("#name").text(data[1].name);
		          $("#recipeIntro").text(data[1].summary);
		          $(".view1_summary_info1").text(data[1].info.info1);
		          $(".view1_summary_info2").text(data[1].info.info2);
		          $(".view1_summary_info3").text(data[1].info.info3);
		          //console.log(data[0].ingre[0].ingre_name);
		          console.log(data[0].ingre.length);
		         for(var i=0;i<data[1].ingre.length;i++){
		        	 var dataString="<h3>"+data[1].ingre[i].ingre_name+"&nbsp;"+data[1].ingre[i].ingre_count+data[1].ingre[i].ingre_unit+"</h3>";
