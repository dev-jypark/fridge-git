...
<script>
	...
	$.ajax({
		//수정파트
		url :"http://192.168.0.5:9080/recipedetail1.do",
		method:"GET",
		dataType:"json",
		success : function(data){
			if(data){
				 console.log(data)
		          $(".view_pic1").css("background-image","url("+data[0].img+")");
		          $("#name").text(data[0].name);
		          $("#recipeIntro").text(data[0].summary);
		          $(".view1_summary_info1").text(data[0].info.info1);
		          $(".view1_summary_info2").text(data[0].info.info2);
		          $(".view1_summary_info3").text(data[0].info.info3);
		          //console.log(data[0].ingre[0].ingre_name);
		          console.log(data[0].ingre.length);
		         for(var i=0;i<data[0].ingre.length;i++){
		        	 var dataString="<h3>"+data[0].ingre[i].ingre_name+"&nbsp;"+data[0].ingre[i].ingre_count+data[0].ingre[i].ingre_unit+"</h3>";
/*		        	 var ingreName = "<h3 id=\"ingre_name"+i+"\">"+data[0].ingre[i].ingre_name+"<h3>";    	 
		...
		error : function(jqWHR,errorThrown){
