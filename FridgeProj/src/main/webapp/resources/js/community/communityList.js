...
function printFeed(data) {
	...
	$.each(data, function(i, feed) {
		...
		feedString += '<img alt="' + feed["nick"] + '" src="/user/profile/' + feed["imgsrc"] + '" class="img-circle pull-left">' + feed["nick"] + '</a></h1>';
		feedString += '<h2 class="pull-right hidden-xs">' + feed["fb_postdate"] + '</h2>';
		feedString += '</header>';

		feedString += '<div class="feedify-item-body">';
		feedString += '<img style="width : 1000px; " alt="' + feed["list_fi_src"][0] + '" src="/upload/feed/' + feed["fb_no"] + '/' + feed["list_fi_src"][0] + '" class="img-responsive"></div>';
		feedString += '<footer>';
...
function insertComment(flag) {
	...
	feeds[index]['list_f_comment'].unshift({"nick":nick, "fc_content":fb_content, "imgsrc":user["imgsrc"]});
	console.log(feeds[index]['list_f_comment']);
	
	if (flag != 0) {
		
		commentString = "";
		commentString += "<div class='content'><img alt='"+nick+"' src='/user/profile/" + user["imgsrc"] + "' class='img-circle pull-left'>";
...
function showModal(index) {
	...
	if ($('.carouselGallery-wrapper').length === 0) {
		if (typeof imagepath !== undefined) {
			...
			modalHtml += "<a href='/memberProfile/" + feed["nick"] + ".do'><img alt='"+feed["nick"]+"' src='/user/profile/" + feed["imgsrc"] + "' class='img-circle pull-left'><span>" + feed["nick"] + "</a></div>";

			modalHtml += "<div class='body'>";
			modalHtml += "<div class='content'><a href='/memberProfile/" + feed["nick"] + ".do'><img alt='" + feed["nick"] + "' src='/user/profile/" + feed["imgsrc"] + "' class='img-circle pull-left'>";
			modalHtml += "<p>" + feed["nick"] +"</a>"+ feed["fb_content"] + "</p></div>";

			// 모달 댓글 출력 파트
			modalHtml += "<div class='comments'>";
			$.each(feed["list_f_comment"], function(i, comment) {
				modalHtml += "<div class='content'><a href='/memberProfile/" + comment["nick"] + ".do'><img alt='"+comment+"' src='/user/profile/" + comment["imgsrc"] + "' class='img-circle pull-left'>";
